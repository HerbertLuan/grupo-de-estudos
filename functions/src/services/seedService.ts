...
import { DEFAULT_BADGES, DEFAULT_LEVELS, DEFAULT_TIMEZONE } from '../config/constants';
import { Group, GroupMember, Season, UserProfile } from '../types';
...
export interface SeedResult {
  ...
  demoCredentials: Array<{ nickname: string; name: string; password: string }>;
}

export async function runSeedDatabase(db: admin.firestore.Firestore): Promise<SeedResult> {
  ...
  for (const m of demoMembers) {
    ...
    // Criação opcional no Firebase Auth para facilitar login imediato no app
    try {
      const email = `${m.nickname.toLowerCase()}@estudos.internal`;
      await admin.auth().getUser(m.uid).catch(async () => {
      ...
      console.warn(`Aviso ao criar usuário Auth para ${m.nickname}:`, authErr.message);
    }
    ...
    // Registro diário: 1h15 (4500s) -> 1 ponto
    await db
      .collection('users')
      .doc(m.uid)
    ...
    // Sessão de estudo concluída recente
    await db.collection('studySessions').doc(`session-${m.uid}-1`).set({
      id: `session-${m.uid}-1`,
      userId: m.uid,
      groupId,
  ...
  return {
    success: true,
    message: 'Ambiente de homologação populado com sucesso!',
    counts: {
      levels: DEFAULT_LEVELS.length,
      badges: DEFAULT_BADGES.length,
      seasons: 1,
      groups: 1,
      members: demoMembers.length,
      users: demoMembers.length,
      feedPosts: 1,
      authUsersCreated,
    },
    demoCredentials: demoMembers.map((m) => ({
      nickname: m.nickname,
      name: m.name,
      password: defaultPassword,
    })),
